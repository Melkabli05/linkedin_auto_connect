Linkedin = {
    config: {
        ...
        addNote: false,
        note: "Bonjour {{name}}, Je serais ravie de faire partie de votre réseau."
    },
    ...
    compile: function (data, config) {
        ...
        if (!data.pageButtons || data.pageButtons.length === 0) {
            ...
            names = [...names].filter(function (element) {
                return
                element.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.par
                entNode.textContent.includes("Connect\n");
            });
    ...
    sendInvites: function (data, config) {
        ...
        if (config.maxRequests == 0) {
            ...
            console.debug('DEBUG: sending invite to ' + (data.pageButtonIndex + 1) + ' out of ' +
    ...
    clickAddNote: function (data, config) {
        ...
        if (addNoteButton && addNoteButton[0]) {
            ...
            console.debug("DEBUG: add note button not found, clicking send on the popup in " +
                config.actionDelay);
            setTimeout(() => this.clickDone(data, config), config.actionDelay);
        }
    },
    pasteNote: function (data, config) {
        ...
        noteTextBox.value = config.note.replace("{{name}}",
    ...
    clickDone: function (data, config) {
        ...
        if (doneButton && doneButton[0]) {
            ...
            console.debug("DEBUG: send button not found, clicking close on the popup in " +
                config.actionDelay);
        }
        setTimeout(() => this.clickClose(data, config), config.actionDelay);
    },
    clickClose: function (data, config) {
        var closeButton = document.getElementsByClassName('artdeco-modal__dismiss artdeco-button artdeco-button--circle artdeco-button--muted artdeco-button--2 artdeco-button--tertiary emberview');
        if (closeButton && closeButton[0]) {
            closeButton[0].click();
        }
        console.info('INFO: invite sent to ' + (data.pageButtonIndex + 1) + ' out of ' +
            data.pageButtonTotal);
        config.maxRequests--;
        config.totalRequestsSent++;
        if (data.pageButtonIndex === (data.pageButtonTotal - 1)) {
            console.debug("DEBUG: all connections for the page done, going to next page in " +
